...
{
  phase_set_by_chromosome_plot_df <- phase_set_summary_tbl %>%
  ...
  write_tsv(phase_set_by_chromosome_plot_df,
            file = file.path(data_dir, "phase_set_by_chromosome_plot_df.tsv"))
...
{
  phase_set_by_sample_plot_df <- phase_set_summary_tbl %>%
    filter(timepoint != "Normal")
  ...
  write_tsv(phase_set_by_sample_plot_df,
            file = file.path(data_dir, "phase_set_by_sample_plot_df.tsv"))
...
{
  phase_set_deletion_plot_df <- phase_sets_tbl %>%
  ...
  write_tsv(phase_set_deletion_plot_df,
            file = file.path(data_dir, "phase_set_deletion_plot_df.tsv"))
...
{
  phase_set_genome_coverage_plot_df <- phase_sets_tbl %>% filter(!is.na(length_variants),
                                                                 length_variants > 0,
                                                                 timepoint != "Normal") %>%
  ...
  write_tsv(phase_set_genome_coverage_plot_df,
            file = file.path(data_dir, "phase_set_genome_coverage_plot_df.tsv"))

  manuscript_numbers[["02_phase_sets"]][["total_genome_coverage_Gb"]] <- phase_set_genome_coverage_plot_df %>% pull(total_length) %>% sum()/1e9
  manuscript_numbers[["02_phase_sets"]][["average_genome_coverage_Gb"]] <- phase_set_genome_coverage_plot_df %>% pull(total_length) %>% sum()/(23*1e9)
  manuscript_numbers[["02_phase_sets"]][["length_group_most_ps"]] <- phase_set_genome_coverage_plot_df %>% group_by(length_factor) %>% summarize(count = count, total = sum(phase_set_genome_coverage_plot_df$count), count_pct = 100*count/total) %>% arrange(count_pct) %>% tail(1)
  manuscript_numbers[["02_phase_sets"]][["0-1_Mb_percentage_of_coverage"]] <- phase_set_genome_coverage_plot_df %>% group_by(length_factor) %>% summarize(total_length = total_length/1e9, total_length_all = sum(phase_set_genome_coverage_plot_df$total_length)/1e9, total_length_pct = 100*total_length/total_length_all) %>% filter(length_factor == "0-1")
  manuscript_numbers[["02_phase_sets"]][["1-2_Mb_percentage_of_coverage"]] <- phase_set_genome_coverage_plot_df %>% group_by(length_factor) %>% summarize(count = count, total_length = total_length/1e9, total_length_all = sum(phase_set_genome_coverage_plot_df$total_length)/1e9, total_length_pct = 100*total_length/total_length_all) %>% filter(length_factor == "1-2")
  manuscript_numbers[["02_phase_sets"]][["n_phase_sets_gt_30Mb"]] <- phase_set_genome_coverage_plot_df %>% filter(length_group >= 31) %>% pull(count) %>% sum()
  manuscript_numbers[["02_phase_sets"]][["longest_ps"]] <- phase_set_genome_coverage_plot_df %>% arrange(length_group) %>% tail(1)
...
# Phase sets by sample
{
  ...
  chr13_chr22_phase_sets_plot_df <- phase_sets_tbl_long %>% bind_rows(phase_sets_tbl_short) %>%
  ...
  write_tsv(chr13_chr22_phase_sets_plot_df,
            file = file.path(data_dir, "chr13_chr22_phase_sets_plot_df.tsv"))

  rm(chr13_chr22_phase_sets_plot_df, phase_sets_tbl_short, phase_sets_tbl_long, plot_df, sample_names, n_samples)
}

# phase set length vs. n heterozygous SNPs
{
  phase_set_length_vs_variants_plot_df <- phase_sets_tbl %>%
  ...
  write_tsv(phase_set_length_vs_variants_plot_df,
            file = file.path(data_dir, "phase_set_length_vs_variants_plot_df.tsv"))
  ...
  phase_set_length_vs_variants.27522_1_plot_df <- phase_sets_tbl %>%
    filter(n_variants_total > 0,
           sample == "27522_1")
  ...
  write_tsv(phase_set_length_vs_variants.27522_1_plot_df,
            file = file.path(data_dir, "phase_set_length_vs_variants.27522_1_plot_df.tsv"))
